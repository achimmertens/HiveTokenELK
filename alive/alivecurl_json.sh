...
#
# Written by Achim Mertens in July 2022
...
ALIVEDOLLAR="$LOGPATH/alivedollar.tmp"
echo "ALIVEDOLLAR = "$ALIVEDOLLAR
...
curl -XPOST -H "Content-type: application/json" -d '{ "jsonrpc": "2.0", "method": "find", "params": { "contract": "market", "table": "tradesHistory", "query": { "symbol": "ALIVE"}, "limit":1000, "offset": 0 }, "id": 1 }' 'https://api.hive-engine.com/rpc/contracts' > $LOG
...
cat $INDEXLOG | awk '{print "{\"index\": {\"_index\":\"alive\",\"_id\":\"" $1 "\"}}="'} > $INDEXLOG2  # Füge Text ein
...
echo "The price of \$USD/\$HIVE = "$HIVEPRICE > $ALIVEDOLLAR
ALIVEPRICELIST=`cat $LOG3 | awk -F'price\":\"' '{print $2}' | awk -F'\"' '{print $1}'`
ALIVEPRICE=`echo $ALIVEPRICELIST | awk -F ' ' '{print $NF}'`
echo "The price of \$HIVE/\$ALIVE = " $ALIVEPRICE
echo ". The price of \$HIVE/\$ALIVE = " $ALIVEPRICE >> $ALIVEDOLLAR
ALIVE_DOLLAR=`echo $ALIVEPRICE \* $HIVEPRICE|bc`
echo "The price of \$USD/\$ALIVE = "$ALIVE_DOLLAR
echo ". The price of \$USD/\$ALIVE = " $ALIVE_DOLLAR >> $ALIVEDOLLAR
sudo cat $ALIVEDOLLAR >> /var/www/html/elk/index.html    # Put the result to the web



# Upload the complete json data into kibana:
curl --location --request POST 'http://localhost:9200/alive/_bulk?' --header 'Content-Type: application/json' --data-binary @$LOGDATE
