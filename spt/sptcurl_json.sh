...
SPTDOLLAR="$LOGPATH/sptdollar.tmp"
echo "SPTDOLLAR = "$SPTDOLLAR
...
curl -XPOST -H "Content-type: application/json" -d '{ "jsonrpc": "2.0", "method": "find", "params": { "contract": "market", "table": "tradesHistory", "query": { "symbol": "SPT"}, "limit":1000, "offset": 0 }, "id": 1 }' 'https://api.hive-engine.com/rpc/contracts' > $LOG
...
cat $INDEXLOG | awk '{print "{\"index\": {\"_index\":\"spt\",\"_id\":\"" $1 "\"}}="'} > $INDEXLOG2  # Füge Text ein
...
echo "The price of \$USD/\$HIVE = "$HIVEPRICE > $SPTDOLLAR
SPTPRICELIST=`cat $LOG3 | awk -F'price\":\"' '{print $2}' | awk -F'\"' '{print $1}'`
SPTPRICE=`echo $SPTPRICELIST | awk -F ' ' '{print $NF}'`
echo "The price of \$HIVE/\$SPT = " $SPTPRICE
echo ". The price of \$HIVE/\$SPT = " $SPTPRICE >> $SPTDOLLAR
SPT_DOLLAR=`echo $SPTPRICE \* $HIVEPRICE|bc`
echo "The price of \$USD/\$SPT = "$SPT_DOLLAR
echo ". The price of \$USD/\$SPT = " $SPT_DOLLAR >> $SPTDOLLAR
sudo cat $SPTDOLLAR >> /var/www/html/elk/index.html    # Put the result to the web
# Upload the complete json data into kibana:
curl --location --request POST 'http://localhost:9200/SPT/_bulk?' --header 'Content-Type: application/json' --data-binary @$LOGDATE
