...
LISTDOLLAR="$LOGPATH/LISTdollar.tmp"
echo "LISTDOLLAR = "$LISTDOLLAR
...
curl -XPOST -H "Content-type: application/json" -d '{ "jsonrpc": "2.0", "method": "find", "params": { "contract": "market", "table": "tradesHistory", "query": { "symbol": "LIST"}, "limit":1000, "offset": 0 }, "id": 1 }' 'https://api.hive-engine.com/rpc/contracts' > $LOG
...
cat $INDEXLOG | awk '{print "{\"index\": {\"_index\":\"list\",\"_id\":\"" $1 "\"}}="'} > $INDEXLOG2  # Füge Text ein
...
curl --location --request POST 'http://localhost:9200/list/_bulk?' --header 'Content-Type: application/json' --data-binary @$LOGDATE
